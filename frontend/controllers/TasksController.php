<?php
    ...
    class TasksController extends SecuredController
    {
        ...
        public function actionView($id)
        {
            ...
            $idPerformer = $task->executor_id; // id исполнителя задания
            $idCustomer = $task->customer_id; // id заказчика задания
            $user_id = Yii::$app->user->identity->getId(); // id текущего пользователя
            $status = $task->status_id;

            $strategy = new Tasks($idPerformer, $idCustomer, $user_id, $status);
            $strategy->getAvailableAction($task->id);

            $responseTaskForm = new ResponseTaskForm($task);
            $completeTaskForm = new CompleteTaskForm();
            $replies = Replies::find()->where(['task_id' => $task->id, 'user_id' => $user_id]);
        ...
        public function actionApply($task_id, $user_id, $reply_id)
        {
            $task = Task::findOne($task_id);
            if ($task->status_id = Status::STATUS_NEW) {
                $task->status_id = Status::STATUS_IN_WORK;
                $task->replies_id = $reply_id;
        ...
        public function actionResponse($task_id)
        {
            $task = Task::findOne($task_id);
            ...
            $replies = Replies::find()->where(['task_id' => $task_id, 'user_id' => $user->id]);
        ...
        public function actionCancel($task_id)
        {
            $task = Task::findOne($task_id);
